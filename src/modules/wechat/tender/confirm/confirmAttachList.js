...
class confirmAttachList extends React.Component {
    constructor(props) {
        super();
        this.state = {
            data:{},
    ...
    componentWillMount(){
        //读取报价页面最底部附件查看进来的数据
        this.setState({
            data:JSON.parse(DataUtil.localSessionRead('confirmAttachList'))
        });
    }
    ...
    attachMgrPdfClick = (data)=>{
        hashHistory.push({
            pathname: '/confirmAttachPdf',
        });
        DataUtil.localSessionRemove('confirmPdfSrc');
        DataUtil.localSessionSave('confirmPdfSrc', {
            confirmPdfSrc: data,
        });
    };
...
export default confirmAttachList;